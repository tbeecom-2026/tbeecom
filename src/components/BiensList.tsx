...
export default function BiensList({
  ...
  useEffect(() => { loadMandats(); }, [search, filtreStatut, filtreType, filtreCategorie, page, onlyWithMandat]);

  useEffect(() => { loadCategories(); }, []);

  async function loadMandats() {
    let query = supabase.from("mandats").select("*", { count: "exact" });
    if (onlyWithMandat) query = query.not("mandat_numero", "is", null);
    ...
    const orderCol = onlyWithMandat ? "mandat_numero_int" : "mandat_numero";
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">{title ?? (onlyWithMandat ? "Mandats" : "Biens")}</h1>
        <div className="flex gap-2">
          ...
          {showCreate && (
            <Button onClick={() => navigate("/mandats/nouveau")}>
              <Plus className="mr-2 h-4 w-4" />Nouveau {onlyWithMandat ? "mandat" : "bien"}
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="overflow-x-auto rounded-lg border border-border">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {mandats.length === 0 && (
              <tr><td colSpan={8} className="p-8 text-center text-muted-foreground">Aucun résultat</td></tr>
            )}
          </tbody>
        </table>
      </div>
