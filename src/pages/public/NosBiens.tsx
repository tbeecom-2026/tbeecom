...
export default function NosBiens() {
  ...
  useEffect(() => {
    Promise.all([
      distinctValues("categorie"),
      distinctValues("commune"),
      distinctValues("type_commerce"),
    ]).then(([c, co, t]) => { setCategories(c); setCommunes(co); setTypes(t); });
  }, []);

  useEffect(() => {
    setLoading(true);
    listPublicBiens({
      search: q || undefined, categorie, type, commune,
    ...
    <>
      <section className="bg-primary text-primary-foreground">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 sm:py-14">
          ...
          <form
            onSubmit={(e) => { e.preventDefault(); const fd = new FormData(e.currentTarget); update({ q: (fd.get("q") as string) || null }); }}
          ...
          >
            ...
            <Button type="submit" size="lg" className="h-12 bg-accent text-accent-foreground hover:bg-accent/90">Rechercher</Button>
            <Button type="button" size="lg" variant="outline" onClick={() => setShowFilters(s => !s)}
              className="h-12 hidden sm:inline-flex border-primary-foreground/30 bg-transparent text-primary-foreground hover:bg-primary-foreground/10">
              <SlidersHorizontal className="h-4 w-4 mr-1" /> Filtres
            </Button>
          </form>
          <Button type="button" variant="outline" onClick={() => setShowFilters(s => !s)}
            className="sm:hidden mt-3 w-full border-primary-foreground/30 bg-transparent text-primary-foreground hover:bg-primary-foreground/10">
            <SlidersHorizontal className="h-4 w-4 mr-1" /> Filtres
          </Button>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        {showFilters && (
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5 mb-6 p-4 rounded-xl bg-card border border-border">
            ...
            <Select value={type} onValueChange={(v) => update({ type: v })}>
              <SelectTrigger><SelectValue placeholder="Type de commerce" /></SelectTrigger>
              <SelectContent>
            ...
            <Input type="number" inputMode="numeric" placeholder="Budget max (€)" defaultValue={prixMax}
              onBlur={(e) => update({ prixMax: e.target.value || null })} />
            <Input type="number" inputMode="numeric" placeholder="Surface min (m²)" defaultValue={surfaceMin}
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr,360px]">
          <div>
            ...
            {loading ? (
              ...
              <div className="grid gap-6 sm:grid-cols-2">
                {items.map(b => <BienCard key={b.id} b={b} />)}
              </div>
            )}
            ...
            {totalPages > 1 && (
              <div className="mt-8 flex justify-center gap-2">
                ...
                <span className="px-4 py-2 text-sm text-muted-foreground">Page {page + 1} / {totalPages}</span>
                <Button variant="outline" disabled={page + 1 >= totalPages} onClick={() => update({ page: String(page + 1) })}>
                  Suivant
                </Button>
              </div>
            )}
                    ...
                    <ul className="mt-4 w-full max-h-48 overflow-auto text-left text-xs space-y-1">
                      {pinned.slice(0, 8).map(b => (
                        <li key={b.id} className="px-3 py-1.5 rounded bg-background/60 truncate">
                          📍 {b.commune}{b.code_postal ? ` · ${b.code_postal}` : ""}
                        </li>
                      ))}
                    </ul>
                  )}
