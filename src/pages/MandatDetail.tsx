              ...
              {!isNew && (
                <label className="cursor-pointer">
                  <input type="file" accept="application/pdf" className="hidden" onChange={async (e) => {
                    ...
                    // Pour l'instant on conserve uniquement le nom du fichier en base.
                    const newDoc = { type: "document", label: file.name.replace(/\.pdf$/i, ''), url: null, date: new Date().toISOString().split('T')[0] };
                    const docs = [...((mandat as any).documents || []), newDoc];
                    const { error: dbErr } = await supabase.from('mandats').update({ documents: docs }).eq('id', id);
                    if (dbErr) { toast({ title: "Erreur sauvegarde", description: dbErr.message, variant: "destructive" }); return; }
                    update("documents", docs);
                    toast({ title: "✅ Document référencé", description: `${file.name} (stockage à rebrancher)` });
                  }} />
