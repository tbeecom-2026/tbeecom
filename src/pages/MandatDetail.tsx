      ...
      <CardContent className="py-5">
        ...
        {isRetire && (
          <div className="mt-4 text-center">
            <Badge variant="secondary" className="text-sm">Mandat retiré</Badge>
          </div>
        )}
      </CardContent>
    </Card>
...
export default function MandatDetail() {
  ...
  const isCommerce =
    !!m.categorie &&
    /fonds de commerce|entreprise/i.test(m.categorie);
  ...
  return (
    <div className="space-y-4 max-w-6xl">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="space-y-2 min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {hasVal(m.reference) && (
                  <span className="text-sm text-muted-foreground">— Réf. {m.reference}</span>
                )}
                {hasVal(m.categorie) && (
              ...
              {hasVal(m.titre) && (
                <h1 className="text-xl font-semibold text-foreground">{m.titre}</h1>
              )}
      ...
      <InfoCard title="Prix & honoraires">
        ...
        {m.murs_a_vendre && (
          <Row label="Prix des murs" value={hasVal(m.prix_murs) ? formatEuros(m.prix_murs) : null} />
        )}
      </InfoCard>
      ...
      <InfoCard title="Bail">
        ...
        <Row label="Charges mensuelles" value={hasVal(m.charges_mensuelles) ? formatEuros(m.charges_mensuelles) : null} />
        <Row label="Droit au bail" value={fmtBool(m.droit_au_bail)} />
        <Row label="Montant droit au bail" value={hasVal(m.montant_droit_bail) ? formatEuros(m.montant_droit_bail) : null} />
